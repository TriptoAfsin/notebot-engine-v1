  ...
  if (process.env.collectAnalytics === "true") {
    try {
      const result = await axios.get(
        `${process.env.analyticsServerUrl}/notes/${subName}?adminKey=${process.env.analyticsAuthKey}`,
...
let handleAnalyticsLabs = async subName => {
  ...
  if (process.env.collectAnalytics === "true") {
    try {
      const result = await axios.get(
        `${process.env.analyticsServerUrl}/labs/${subName}?adminKey=${process.env.analyticsAuthKey}`,
        console.log(`🟢 Analytics was handled successfully`)
      );
    } catch (err) {
      //console.log(err)
...
const labFlow_level4 = require("./flows/botReplies/lab_levels/level_4/level_4_lab_flow");
...
let  handleMessage = async (sender_psid, received_message) =>  {
  ...
  */
  ...
  //food
    ...
    wordIncludes(foodWords, received_message) ||
    wordIncludes(khawaWords, received_message)
  ) {
  ...
  //corona
    ...
    let loadingResponse = textBlockGen(
      "🟡 Please wait, looking for latest updates ..."
    );
  ...
  //phone
    ...
    response = groupedBtnBlockGen(`🔰BUTEX PhoneBook - `, [
      webBtnBlockGen(
        `🌍 Visit Here`,
        `https://triptoafsin.github.io/butex-phonebook-v2/`
      ),
    ]);
    callSendAPI(sender_psid, response);
  }
...
let handlePostback = async (sender_psid, received_postback) => {
  ...
  //manza
  else if (payload === "manza_catalogue") {
  ...
  else if (payload === "level_4_lab") {
    magicFunc(sender_psid, labFlow_level4);
  }
